...
class TrackerSiamRPN(Tracker):
    ...
    def __init__(self, net, net_path=None, **kargs):
        ...
        self.response_threshold_h = 0.8
        self.response_threshold_l = 0.5
        self.response_h_cnt = 0
        self.response_l_cnt = 0
        self.cnt_h_threshold = 25
        self.cnt_l_threshold = 6
    ...
    def update(self, image, get_loss=False):
        t1 = time.time()
        image = np.asarray(image)
        losses = 0
        ratio = 0
        ...
        # f = cv2.FileStorage('update.yml',flags=1)
        
        # f.write('update_matrix', instance_image)
        # f.release()
        ...
        with torch.set_grad_enabled(False):
            self.net.eval()
            kr = self.kernel_reg.cpu().numpy()
            kc = self.kernel_cls.cpu().numpy()
            kr = torch.from_numpy(kr).to(self.device)
            kc = torch.from_numpy(kc).to(self.device)
            out_reg, out_cls = self.net.inference(
                instance_image, kr, kc)
        ...
        box = [
            max(0, int(self.center[1] + 1 - (self.target_sz[1] - 1) / 2)),
            max(0, int(self.center[0] + 1 - (self.target_sz[0] - 1) / 2)),
            max(0, int(self.target_sz[1])), max(0, int(self.target_sz[0]))]
        ...
        if max(response) > self.response_threshold_h:
            return True, box, losses
        elif max(response) > self.response_threshold_l:
            self.response_l_cnt = 0
            self.response_h_cnt += 1
        else:
            self.response_h_cnt = 0
            self.response_l_cnt += 1
        if self.response_h_cnt > self.cnt_h_threshold and self.response_l_cnt == 0:
            return False, box, 1
        elif self.response_l_cnt > self.cnt_l_threshold and self.response_h_cnt == 0:
            return False, box, 1
        else:
            return True, box, losses
    ...
    def _create_anchors(self, response_sz):
        ...
        for ratio in self.cfg.ratios:
            ...
            for scale in self.cfg.scales:
                ...
                ind += 1
        anchors = np.tile(
            anchors, response_sz * response_sz)
...
if __name__ == '__main__':
    cap = cv2.VideoCapture('/Users/wangxu/Edge/EdgeTileClient/data/drone/video_720p/uav0000295_02300_v.mp4')
    i = 0
    while cap.isOpened():
        ret, frame = cap.read()
        if ret == True:

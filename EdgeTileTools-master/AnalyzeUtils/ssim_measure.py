...
if __name__ == "__main__":
    ...
    x = [0, 10, 20, 30, 40, 50]
    y = [
            np.mean([x[3] for x in hevc_encode_862_0_list]),
            np.mean([x[3] for x in hevc_encode_862_10_list]),
            np.mean([x[3] for x in hevc_encode_862_20_list]),
            np.mean([x[3] for x in hevc_encode_862_30_list]),
            np.mean([x[3] for x in hevc_encode_862_40_list]),
            np.mean([x[3] for x in hevc_encode_862_50_list])
        ]
    ax.plot(y, x, linestyle='-', color='green', marker= 'o', label = "HEVC")
    x = [0, 20, 40, 60, 80, 100]
    y = [
            np.mean([x[4] for x in jpeg_encode_862_0_list]),
            np.mean([x[4] for x in jpeg_encode_862_20_list]),
            np.mean([x[4] for x in jpeg_encode_862_40_list]),
            np.mean([x[4] for x in jpeg_encode_862_60_list]),
            np.mean([x[4] for x in jpeg_encode_862_80_list]),
            np.mean([x[4] for x in jpeg_encode_862_100_list])
        ]
    ...
    ax.grid()
    ax.set_xlabel("Average SSIM")
    ax.set_ylabel("qp(HEVC encode)", color='green')
    ax2.set_ylabel("quality(JPEG encode)", color='blue')
    plt.title('Relationship between qp&quality and SSIM.(720p)') 
    fig.legend(loc='best', bbox_to_anchor=(1,1), bbox_transform=ax.transAxes)
    plt.savefig("analyze_result/720p-qp-quality-ssim.png", bbox_inches='tight', dpi=300)
    ...
    plt.title('Relationship between SSIM and time cost.(720p)') 
    plt.savefig("analyze_result/720p-ssim-time.png", bbox_inches='tight', dpi=300)
    ...
    x = [0, 10, 20, 30, 40, 50]
    y = [
            np.mean([x[3] for x in hevc_encode_2464_0_list]),
            np.mean([x[3] for x in hevc_encode_2464_10_list]),
            np.mean([x[3] for x in hevc_encode_2464_20_list]),
            np.mean([x[3] for x in hevc_encode_2464_30_list]),
            np.mean([x[3] for x in hevc_encode_2464_40_list]),
            np.mean([x[3] for x in hevc_encode_2464_50_list])
        ]
    ax.plot(y, x, linestyle='-', color='green', marker= 'o', label="HEVC")
    
    x = [0, 20, 40, 60, 80, 100]
    y = [
            np.mean([x[4] for x in jpeg_encode_2464_0_list]),
            np.mean([x[4] for x in jpeg_encode_2464_20_list]),
            np.mean([x[4] for x in jpeg_encode_2464_40_list]),
            np.mean([x[4] for x in jpeg_encode_2464_60_list]),
            np.mean([x[4] for x in jpeg_encode_2464_80_list]),
            np.mean([x[4] for x in jpeg_encode_2464_100_list])
        ]
    ...
    ax.set_ylabel("qp(HEVC encode)", color='green')
    ax2.set_ylabel("quality(JPEG encode)", color='blue')
    plt.title('Relationship between qp&quality and SSIM.(2k)') 
    fig.legend(loc='best', bbox_to_anchor=(1,1), bbox_transform=ax.transAxes)
    plt.savefig("analyze_result/2k-qp-quality-ssim.png", bbox_inches='tight', dpi=300)
    ...
    ax.set_xlabel("Average SSIM")
    ax.set_ylabel("Average encode time cost(ms)")
    plt.title('Relationship between SSIM and time cost.(2k)') 
    plt.savefig("analyze_result/2k-ssim-time.png", bbox_inches='tight', dpi=300)

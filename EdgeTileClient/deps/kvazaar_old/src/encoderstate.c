...
static void encoder_state_worker_encode_lcu(void * opaque)
{
  ...
  //fprintf(stdout, "real encoding lcu: %d, slice: %d\n", lcu->id, state->slice->id);
    ...
    if (node_is_the_last_split_in_tree) {
      for (int i = 0; main_state->children[i].encoder_control; ++i) {
        ...
        if (main_state->children[i].type != ENCODER_STATE_TYPE_WAVEFRONT_ROW) {
          ...
          //printf("task upload:%d\n", main_state->children[i].slice->id);
...
{
  //printf("start encoding one frame\n");
  ...
  //printf("start encoding one frame2\n");
  ...
  for (int i = 0; state->children[i].encoder_control; i++) {
    ...
    //printf("start encoding one frame4\n");
  }
